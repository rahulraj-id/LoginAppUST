...
class AuthRepositoryImpl @Inject constructor(private val prefs: Prefs, private val manager: OAuthManager) : AuthRepository {
    ...
    override fun handleCode(code: String): Boolean { val token = manager.exchange(code); if (token.accessToken.isEmpty()) return false; prefs.saveToken(token.accessToken, token.refreshToken); return true }
    override fun silentAuth(): Boolean { val refresh = prefs.getRefreshToken(); val access = prefs.getAccessToken(); if (refresh != null) { val t = manager.refresh(refresh); if (t.accessToken.isEmpty()) return false; prefs.saveToken(t.accessToken, t.refreshToken); return true } return access != null }
